...
/*
 * 53-transparent-proxy-v3: 业务服务进程（纯 socket，无 BPF 侵入）。
 *
 * 监听 127.0.0.1:9000（非 :8080，避免 sk_lookup 回环）。
 * 收到 GET /outbound 时主动 connect 192.168.99.2:9090（演示出流量劫持）。
 *
 * 外部 client 仍用 curl 127.0.0.1:8080 访问（sk_lookup 透明拦截到 sidecar）。
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(void)
{
	...
	if (srv_fd < 0)
		return 1;
	printf("[server] listening on 127.0.0.1:%d (virtual :%d hijacked by sk_lookup)\n",

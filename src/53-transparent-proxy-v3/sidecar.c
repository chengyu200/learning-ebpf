...
/*
 * 53-transparent-proxy-v3: 用户态 sidecar。
 *
 * 双钩子架构：
 *   - sk_lookup（netns）→ 入流量劫持（外部 client 无需在 cgroup）
 ...
 *   - 无条目 → 入流量（外部 client 被 sk_lookup 重定向到此）→ connect(:9000)
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc, char **argv)
{
	...
	while (!exiting) {
		...
		if (bpf_map_lookup_elem(conn_map_fd, &ck, &orig) == 0) {
			...
			/* 入流量：外部 client 被 sk_lookup 重定向到此 → 回源到 :9000 */
			up_addr.sin_family = AF_INET;

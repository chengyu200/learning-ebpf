...
/*
 * 62-iter-cgroup: 用户态 — 遍历 cgroup 层级。
 *
...
int main(int argc, char **argv)
{
	...
	skel = iter_cgroup_bpf__open_and_load();
	if (!skel) {
	...
	link = bpf_program__attach_iter(skel->progs.dump_cgroup, &opts);
	if (!link) {
	...
	printf("=== Cgroup Hierarchy (root=%s, order=%s) ===\n", cg_path, order_str);
